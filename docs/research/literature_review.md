...
## Cluster 2 — Fast-switching / blinking-network theory (and its correct limits)

Belykh–Belykh–Hasler (2004) introduced the "blinking" model and showed that **fast on/off links synchronize a network as if it were coupled by its time-averaged topology**. Stilwell–Bollt–Roberson (2006) proved the sufficient condition: *if the time-averaged network synchronizes, sufficiently fast switching does too*, even when no instantaneous snapshot does. Hasler–Belykh–Belykh (2013) bound the finite-time deviation from the averaged system. Porfiri et al. (2006) and Frasca et al. (2008) extend this to proximity/mobility-driven time-varying links.

**Corrected synthesis (this is a change from the v1 review — see the audit).** Averaging theory says only this: *in the fast-switching limit, a temporal network behaves like its time-average.* It does **not** say that a temporal network can never beat a static one. **Zhang & Strogatz (2021, Nature Comms 12, 3273)** prove the opposite is possible: there exist edge/resource budgets under which **no static network synchronizes but a well-designed temporal network does**, and the advantage (i) appears at **intermediate** switching rates (it is non-monotonic in rate — the fast limit merely recovers the average), and (ii) is governed by the **curvature of the MSF**. Two consequences for this project:

1. The fast limit (average-graph dominance) is the *least* favourable regime for a temporal advantage; observing it there says nothing about intermediate rates.
2. The right static comparator is not the time-average but the **best static network admissible under the same budget** — which may itself fail to synchronize, opening room for a genuine temporal advantage.

**Relevance:** this cluster still explains the seed paper's fast-switching result and our reproduction. But it **corrects our v1 over-claim** that "switching never beats the average graph": that observation was made only in the fast/slow regime with a confounded contrast (audit D4/D5) and with the time-average (not the best admissible static) as comparator, so it cannot be generalized. The open question — does a curvature-driven intermediate-rate advantage exist over the best admissible static network — is deferred to a separate, unexecuted branch (`intermediate_rate_temporal_advantage_prereg_v1.md`).
...
1. **Averaging theory constrains only the fast-switching limit.** In that limit a temporal network behaves like its time-average, so a temporal advantage is not expected *there*. It does **not** follow (as the v1 review wrongly implied) that switching can never beat static networks: Zhang–Strogatz (2021) show a budget-constrained, curvature-driven advantage at **intermediate** rates over the **best admissible static** comparator. The correct open question is therefore rate- and comparator-specific, not settled by averaging theory.
...
*(Correction note: the v1 version of this section over-generalized averaging theory into "switching will not beat the average graph." That claim is withdrawn; see `docs/audits/p1_v1_independent_audit.md`.)*
