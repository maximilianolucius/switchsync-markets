...
> - **Pre-execution status (P1.2-F):** the authoritative harness is prereg v8 + execution contract v6 + execution freeze v8 (`switchsync-synthetic-execution-v8-freeze`); freezes v3/v4/v5/v6/v7 were each found non-executable and superseded (v7 lacked the `tolerances` block, so `--plan` raised `KeyError`). v8 restores exactly the v3 tolerances and changes nothing else scientific. A single cheap-suite one-shot is authorized under P1.2-F; its results are kept OUTSIDE the repo (sealed, not committed).
> - **Pre-execution status (P1.2-B):** runners exist but are NOT executed. The v3 execution freeze was found non-executable and is superseded by prereg v4 + execution contract v2 + execution freeze v4 (`switchsync-synthetic-execution-v4-freeze`). No v2/v3/v4 scientific result exists.
