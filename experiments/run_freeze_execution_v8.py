"""Build and verify the EXECUTION freeze v8 (P1.2-F). Supersedes freeze v7.

Minimal-correction freeze: prereg v8 (restores the v3 `tolerances` block) +
execution contract v6, plus the v7->v8 changelog, the P1.2-E audit and the freeze v7
SUPERSEDED sidecar, on top of every preserved earlier prereg/contract/sidecar/audit/
report, the lockfile, python version and the source PDF SHA. Writes
artifacts/freeze_execution_v8.json. NO real data."""
...
SPEC = {
    ...
    "files": [
        ...
        "experiments/configs/synthetic_prereg_v8.json",
        "experiments/configs/synthetic_execution_contract_v6.json",
        ...
        "docs/methodology/synthetic_prereg_v8.md",
        "docs/methodology/prereg_v7_to_v8_changelog.md",
        ...
        "artifacts/freeze_execution_v7.SUPERSEDED.md",
...
CONFIG_REL = "experiments/configs/synthetic_prereg_v8.json"
PDF_REL = "docs/research/sources/eser2025_arxiv_2507.08007v2.pdf"
OUT = "artifacts/freeze_execution_v8.json"
...
def main() -> None:
    ...
    print(f"execution freeze v8 written: {manifest['n_files']} files -> {out}")
    print(f"prereg v8 canonical : {manifest['config_canonical_hash']}")
    print(f"prereg v8 file SHA  : {manifest['config_file_sha256']}")
