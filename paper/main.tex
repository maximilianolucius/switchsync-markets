...
\author{SwitchSync Markets Project\thanks{Repository:
\texttt{switchsync-markets}. This is a fully synthetic, preregistered study;
no real market data were used at any stage. Contact via the repository.}}
...
with diagnostically negative differences in all eight seeds; no temporal-order
effect was resolved; robustness stages never exceeded their bands; and
observational identifiability failed outright (precision and recall
...
Eser, Medeiros, Riza and Engel recently showed that switching a sparse set of
inter-layer links --- rather than keeping them fixed --- can stabilize
inter-layer synchronization in two-layer networks of chaotic FitzHugh--Nagumo
oscillators, with faster switching more effective
...
\begin{enumerate}
  ...
  the classic interdependence/heteroskedasticity confound
  \citep{forbes2002no}.
...
Synchronization is declared when the tail time-average (final 25\%) of the
inter-layer error $E_{12}$ falls below $0.02$; both tolerances were frozen in
the preregistration. G0B uses $N=40$, $\sigma_{12}=1.5$, total time $800$,
...
\begin{enumerate}
  ...
  frozen floors ($0.02$ for surrogate $\gamma$ and FHN $E_{12}$; $0.05$ for
  the identifiability margin);
  \item decision: if $p\ge\alpha$ $\to$ INCONCLUSIVE (NOT\_SIGNIFICANT); else
  if $\bar d> b$ $\to$ PASS; else if $\bar d<-b$ $\to$ FAIL; else
  $\to$ INCONCLUSIVE (TIE).
\end{enumerate}
A bootstrap interval (2{,}000 resamples, frozen seed) is reported as a
\emph{descriptor only} and plays no role in decisions. A global failed-seed
...
($\Tswt\ge160$) do not (fraction $0.2$), with a monotone descent in between
(Table~\ref{tab:g0b}, Figure~\ref{fig:g0b}). Five of five seeds succeeded in
...
\section{Why Aggregate Connectivity Dominates the Interpretation}
\label{sec:aggregate}
...
Even in the most favorable conditions finance can never offer --- known
synthetic dynamics, same-realization ground truth, a factor-free estimator by
construction, and synchronous noise-controlled observations --- channel-set
recovery reached precision and recall of only $0.353$. Beating the
...
(v) an independent audit that reconstructed the package and recomputed all
identities, hashes and decision statistics exactly;
...
were each found non-executable by independent audits and were \emph{never
run}; consequently no result predates the sealed attempt. All identifiers
...
Execution freezes v2--v7 were each found non-executable by independent
pre-execution audits (defects ranged from custody flaws to a missing frozen
tolerance block) and were superseded without ever producing a result. The
